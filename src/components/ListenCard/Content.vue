...
import activeImage from "~/assets/icons/active.gif"
...
const allItems = Array.from(Array.from({ length: 999 }).keys())
...
<template>
  <div class="h-full">
    ...
    <div class="absolute left-0 w-[200px] h-[calc(100%-80px-64px-64px)] border-r border-gray-200 bg-inherit py-2">
      ...
      <div v-else v-bind="containerProps" :style="{ height: `${window.innerHeight - 80 - 64 - 64 - 80}px` }">
        <div v-bind="wrapperProps">
          <div v-for="item in list" :key="item">
            <div
              :class="[ 'px-4 py-4 cursor-pointer hover:bg-primary/10', { 'bg-primary/30': index === currentIndex }]"
              @click="emit('video-change', 'currentTime', index)"
            >
              <div class="flex items-center justify-between">
                <span class="text-gray-600">第 {{ item + 1 }} 句</span>
                <img v-if="index === currentIndex && isPlaying" :src="activeImage" alt="active indicator"
                  class="w-4 h-4" />
                <img v-else-if="index === currentIndex" class="w-4 h-4"
    ...
    <div class="flex justify-between items-center ml-[200px] px-12 h-full">
      <Button size="icon" class="rounded-full h-12 w-12 min-w-12"
        @click="currentIndex > 0 && emit('video-change', 'currentTime', currentIndex - 1)" title="快捷键:Shift+左箭头⬅"
      ...
      <div class="text-lg px-10 py-4 space-y-4 flex flex-col align-middle flex-grow h-full">
        ...
        <div class="flex flex-col items-center space-y-4">
          <!-- 原文部分 -->
          <div class="">
            <div class="pl-4" @click.stop="handleChange(currentIndex, 'showRaw', !currentRowData?.showRaw)">
              <div class="min-h-[40px] flex cursor-pointer tems-center rounded-lg">
                <span class="text-base" :style="currentRowData?.showRaw ? {} : {
        color: 'transparent',
        textShadow: '#5a5656 0 0 11px'
          ...
          <!-- 译文部分 -->
          <div class="">
            <div class="pl-4" @click.stop="handleChange(currentIndex, 'showTranslate', !currentRowData?.showTranslate)">
              <div :class="[
        'min-h-[48px] flex items-center cursor-pointer rounded-lg',
        currentRowData?.showTranslate ? '' : 'backdrop-blur-md bg-white/30'
      ]">
                <span class="text-base" :style="currentRowData?.showTranslate ? {} : {
                 color: 'transparent',
        textShadow: '#5a5656 0 0 11px'
      }">
        ...
        <!-- 听写输入区域 -->
        <!-- <div v-if="listenWriteMode" id="listenWriteContentDom" class="flex flex-wrap gap-3 w-full">
          <template v-if="currentRawArr.length">
            <InputWord v-for="(item, index) in currentRawArr" :key="`${item}-${index}`"
              :set-focus-index="(index) => focusIndex = index" :focus-index="focusIndex" :word="item" :index="index"
          ...
          <div v-else class="w-full flex justify-center">
            <Input class="w-[80%] min-h-[60px]" :value="currentRowData?.listenWriteContent || ''" @change="(e: { stopPropagation: () => void; target: { value: string | boolean } }) => {
        e.stopPropagation()
        ...
        <!-- <div v-if="noteMode" class="w-full flex flex-col items-center">
          <Textarea class="w-[80%] min-h-[80px]" placeholder="笔记...(可导出笔记)" :value="currentRowData?.noteContent || ''"
            @change="(e: { stopPropagation: () => void; target: { value: string | boolean } }) => {
        e.stopPropagation()
      ...
      <Button size="icon" class="rounded-full h-12 w-12 min-w-12"
        ...
        title="快捷键:Shift+右箭头➡" :disabled="currentIndex === totalLength - 1">
        <ArrowRight />
      </Button>
    </div>
  </div>
